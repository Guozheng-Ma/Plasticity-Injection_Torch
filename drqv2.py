...
class Critic(nn.Module):
    def __init__(self, repr_dim, action_shape, feature_dim, hidden_dim):
        ...
        self.original_Q1 = nn.Sequential(
            nn.Linear(feature_dim + action_shape[0], hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, 1))
        self.original_Q2 = nn.Sequential(
            nn.Linear(feature_dim + action_shape[0], hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, 1))

        self.delta_Q1 = nn.Sequential(
            nn.Linear(feature_dim + action_shape[0], hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, 1))
        self.delta_Q2 = nn.Sequential(
            nn.Linear(feature_dim + action_shape[0], hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, 1))

        self.bias_Q1 = nn.Sequential(
            nn.Linear(feature_dim + action_shape[0], hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, 1))
        for param in self.bias_Q1.parameters():
            param.requires_grad = False
        self.bias_Q2 = nn.Sequential(
            nn.Linear(feature_dim + action_shape[0], hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(inplace=True), nn.Linear(hidden_dim, 1))
        for param in self.bias_Q2.parameters():
            param.requires_grad = False
    ...
    def plasticity_injection(self):
        self.use_pi = True
        for param in self.original_Q1.parameters():
            param.requires_grad = False
        for param in self.original_Q2.parameters():
            param.requires_grad = False
    ...
    def forward(self, obs, action):
        ...
        if self.use_pi:
            # original
            original_q1 = self.original_Q1(h_action)
            original_q2 = self.original_Q2(h_action)
            # delta
            delta_q1 = self.delta_Q1(h_action)
            delta_q2 = self.delta_Q2(h_action)
            # bias
            bias_q1 = self.bias_Q1(h_action)
            bias_q2 = self.bias_Q2(h_action)
            # compute
            # q1 = original_q1.detach() + delta_q1 - bias_q1.detach()
            # q2 = original_q2.detach() + delta_q2 - bias_q2.detach()  
            q1 = original_q1 + delta_q1 - bias_q1
            q2 = original_q2 + delta_q2 - bias_q2
        else:
            q1 = self.original_Q1(h_action)
            q2 = self.original_Q2(h_action)
...
class DrQV2Agent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
                 hidden_dim, critic_target_tau, num_expl_steps, use_aug,
